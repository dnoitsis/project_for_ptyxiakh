from functions_didaktoriko import createTrafficMatrix, return_critical_nodes, return_critical_regions, createTrafficMatrix2, return_critical_links, bfs, bfs_avoid_critical
...
for i in nodes:
    for j in nodes:
        if i != j:
            path = bfs_avoid_critical(i, j, g,critical_links)
...
for i in range(1, 7):
    ...
    for j in range(test_times):
        ...
        # sum_of_EAFFB_30 += EAFFB(nodes, links, g, traffic, regionLinks, 0.3)/1000
        # sum_of_EAFFB_40 += EAFFB(nodes, links, g, traffic, regionLinks, 0.4)/1000
        # sum_of_EAFFB_50 += EAFFB(nodes, links, g, traffic, regionLinks, 0.5)/1000

        eaffb, dummy_num_rooters, dummy_num_tran, dummy_num_edfa = EAFFB(nodes, links, g, traffic, regionLinks, 0.3)
        sum_of_EAFFB_30 += eaffb

        # sum_of_EAFFB2_30 += EAFFB2(nodes, links, g, traffic, regionLinks, 0.3)/1000
        # sum_of_EAFFB2_40 += EAFFB2(nodes, links, g, traffic, regionLinks, 0.4)/1000
        # sum_of_EAFFB2_50 += EAFFB2(nodes, links, g, traffic, regionLinks, 0.5)/1000
    ...
    print("EAFFB 40%: " + str(sum_of_EAFFB_40 / test_times))
    print("EAFFB 50%: " + str(sum_of_EAFFB_50 / test_times))

    print("EAFFB2 30%: " + str(sum_of_EAFFB2_30 / test_times))
    print("EAFFB2 40%: " + str(sum_of_EAFFB2_40 / test_times))
    print("EAFFB2 50%: " + str(sum_of_EAFFB2_50 / test_times))
